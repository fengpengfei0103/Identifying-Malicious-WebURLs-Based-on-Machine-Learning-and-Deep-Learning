...
if __name__ == '__main__':
    ...
    # y_tensor = torch.tensor(y[:1310500], dtype=torch.float32)
    print(X_tensor)
    print("PyTorch张量")
    ...
    # 保存图表到当前目录下
    plt.savefig('FCnet_convergence_plot.png')  # 你可以指定其他文件格式如 '.pdf', '.svg', '.eps' 等
    ...
    # 导出模型为ONNX格式
    torch.onnx.export(model, dummy_input, 'FCnet_model.onnx', input_names=['input'], output_names=['output'])
    ...
    # 保存混淆矩阵到本地
    plt.savefig('FCnet_confusion_matrix.png')
